import { DATABASE_NAME, SOCIAL_MEDIA_PLATFORM, socialBaseUrl } from '@constants/index';
...
export class AuthService {
  constructor(
    @InjectModel(SocialMediaAccount.name, DATABASE_NAME) private socialMediaAccountModel: Model<SocialMediaAccount>
  ) {}
  ...
  async saveFacebookAccessToken(workspaceId: string, accessToken: string) {
    try {
      const longLivedAccessToken = await this.exchangeForLongLivedFacebookToken(accessToken);
